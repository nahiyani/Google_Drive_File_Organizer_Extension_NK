...
async function getOrganizationPlan(preset, files) {
    ...
    const response = await fetch(
      "https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.2",
      {
        ...
        headers: {
          ...
          // Note: YES I EXPOSED THE API KEY. NOOOOOOOOOO! Dont care its free..
...
// Fallback option
function getFallbackOrganizationPlan(preset, files) {
  switch (preset) {
    case "preset1":
...
async function executeOrganizationPlan(plan, parentFolderId, token) {
  const folderPromises = plan.folders.map(async (folder, index, array) => {
    ...
    const movePromises = folder.files.map(async (fileName, fileIndex) => {
      const fileObj = fileListData.files.find((f) => f.name === fileName);
